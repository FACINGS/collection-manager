...
export function CollectionItemsList({ collections, title, author }) {
  ...
  const showMoreParameters = {
    ...
    match: search,
    service:
      search.length > 0 ? searchCollectionService : listCollectionsService,
  };

  async function handleSearchInput(e) {
    setSearch(e.target.value);
    if (e.target.value.length > 0) {
      const { data: match } = await searchCollectionService({
    ...
    if (e.target.value.length == 0) {
      if (author) {
        const { data: collections } = await collectionsByAuthorService({
          author,
        });
