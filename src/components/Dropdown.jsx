  ...
  const optionsList = options.map((option, index) => {
    return (
      <div
        ...
        className={`text-white px-lg py-md whitespace-nowrap text-center ${
          option.onClick ? 'hover:bg-secondary hover:cursor-pointer' : null
        }`}
      >
        {option.label}
      </div>
    ...
    >
      <div
        ...
        className={`text-lg font-bold ${
          !disabled
            ? 'hover:-translate-y-0.5 hover:transition-all cursor-pointer'
            : ''
        }`}
      >
        {label}
      ...
      <div
        className={`${
          open ? 'block' : 'hidden'
        } absolute bg-primary w-fit rounded z-10 drop-shadow-lg top-[40px] ${
