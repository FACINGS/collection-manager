...
const Login = ({ ual }) => {
  ...
  if (accountInformation) {
    availableRam = (
      ((accountInformation.ram.quota - accountInformation.ram.used) * 100) /
      accountInformation.ram.quota
    ).toFixed(2);
  }
  ...
  if (!ual?.activeUser) {
    return (
      <button className="text-lg" onClick={ual?.showModal}>
        Login
      </button>
    );
  }
  ...
  return (
    <Dropdown label={ual.activeUser.accountName} options={dropdownOptions} />
  );
};
