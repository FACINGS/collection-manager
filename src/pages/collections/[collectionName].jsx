...
export default function Collection({ collection, stats, templates, assets, schemas, accounts }) {
    ...
    function handleTabContent(tab) {
        switch (tab) {
            ...
            case 'Templates':
                return <CollectionTemplatesList templates={templates} collection={collection.collection_name} totalTemplates={stats.templates} />;
                break;

            case `NFT's`:
                return <CollectionAssetsList assets={assets} collection={collection.collection_name} totalAssets={stats.assets} />;
                break;

            case `Schemas`:
                return <CollectionSchemasList schemas={schemas} collection={collection.collection_name} totalSchemas={stats.schemas} />;
                break;

            case `Accounts`:
                return <CollectionAccountsList accounts={accounts} collection={collection.collection_name} />;
                break;
            ...
            <div className="flex flex-col lg:flex-row items-center md:items-start">
                <div className="flex flex-col items-center m-4 w-fit h-fit">
                    <div className="bg-primary w-full flex justify-center rounded-t-lg overflow-hidden">
                        { collection.img ? (
                            <Image alt={collection.name} src={`${ipfsEndpoint}/${collection.img}`} width="320" height="320" objectFit="contain"/>
                        ) : (
                    ...
                    <div className={`w-auto max-w-prose h-fit shadow-lg p-8 min-w-[20rem] rounded-b-lg flex flex-col bg-primary ${collection.data.description ? 'gap-4' : null }`}>
                        <div className="flex flex-col gap-2 text-white">
                            <h1 className="text-2xl font-bold">{collection.name}</h1>
                            <div className="flex gap-2 items-center">
                                <p className="text-sm">Author:</p>
                                <Link href={`author/${collection.author}`}>
                                    <a className="text-sm font-bold whitespace-nowrap cursor-pointer underline">{collection.author}</a>
                                </Link>
                            </div>
                            <div className="flex gap-2 items-center">
                                <p className="text-sm">URL:</p>
                                <a href={collection.data.url} target="_blank" rel="noopener noreferrer" className="underline decoration-solid text-sm font-bold whitespace-nowrap">{collection.data.name}</a>
                            </div>
                            <div className="flex gap-2 items-center">
                                <p className="text-sm">Created:</p>
                                <p className="text-sm font-bold whitespace-nowrap">{handleTimestamp()}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col p-8 m-4 bg-white  w-auto md:w-fit shadow-lg rounded-lg gap-8 min-w-[20rem]">
                    ...
                    <Tabs tabs={tabs}>
                        {({ selectedTab }) => (                    
                            <>
                                {tabs.map((item, index) => (
                                    <div key={index} style={{ display: selectedTab === item ? 'block' : 'none' }}>
...
export async function getStaticProps(context) {
    ...
    const { data:templates } = await collectionTemplatesService({ collection: collectionName });
    const { data:assets } = await collectionAssetsService({ collection: collectionName });
    const { data:schemas } = await collectionSchemasService({ collection: collectionName });
    const { data:accounts } = await collectionAccountsService({ collection: collectionName });
