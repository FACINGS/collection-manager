  ...
  function handleImage(collection) {
    if (collection?.data?.video) {
      return (
        ...
        >
          <source
            src={`${ipfsEndpoint}/${collection.data.video}`}
            type="video/mp4"
          />
        </video>
      );
    ...
    if (collection.data.img) {
      return (
        <Image
          alt={collection.name}
          src={`${ipfsEndpoint}/${collection?.data?.img}`}
          width="160"
          height="160"
          objectFit="cover"
        />
      ...
      <div className="w-fit flex flex-row flex-wrap justify-center md:justify-start max-w-screen-md bg-white drop-shadow-md rounded-lg">
        {collections.map((collection, index) => {
          return (
            <div
              key={index}
              className="flex flex-col items-center m-4 w-fit h-fit cursor-pointer"
              onClick={() =>
            ...
            >
              ...
              <div
                className={`w-40 h-fit shadow-md p-4 max-w-40 rounded-b-lg flex flex-col bg-primary text-white text-center items-center relative`}
              >
