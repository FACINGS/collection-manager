...
const wax = new waxjs.WaxJS({ rpcEndpoint: `${waxChain.rpcEndpoints[0].protocol}://${waxChain.rpcEndpoints[0].host}` });

export async function getWaxAccountInfo(activeUser, onInfoResponse = () => {}, error = () => {}) {
    try {
        const response = await wax.rpc.get_account(activeUser.accountName);

        const data = {
            waxAvailable: response.core_liquid_balance ? response.core_liquid_balance : '0.00000000 WAX',
            cpu: {
